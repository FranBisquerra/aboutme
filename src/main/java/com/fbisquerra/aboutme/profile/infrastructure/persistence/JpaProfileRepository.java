...
import com.fbisquerra.aboutme.profile.infrastructure.persistence.entity.ProfileCourseJpaEntity;
...
public class JpaProfileRepository implements ProfileRepository {
    ...
    public void save(Profile profile) {
        ...
        replace(entity.getCourses(), profile.courses(),
            c -> new ProfileCourseJpaEntity(c.institution(), c.name(), c.start(), c.end()));
    ...
    private Profile toDomain(ProfileJpaEntity entity) {
        return new Profile(
            ...
            entity.getCourses().stream()
                .map(c -> new Profile.CourseEntry(c.getInstitution(), c.getName(), c.getStartYear(), c.getEndYear()))
                .toList()
        );
    }
}
